...
export const FileCTR: MessageFns<FileCTR> = {
  encode(message: FileCTR, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): FileCTR {
  ...
  toJSON(message: FileCTR): unknown {
    const obj: any = {};
    if (message.deleted !== false) {
  ...
  create(base?: DeepPartial<FileCTR>): FileCTR {
    return FileCTR.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<FileCTR>): FileCTR {
    const message = createBaseFileCTR();
